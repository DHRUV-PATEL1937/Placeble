...
export function GroupDiscussion({ accessToken, onBack }: { accessToken: string; onBack: () => void }) {
  ...
  const requestMicrophone = async () => {
    ...
    try { streamRef.current?.getTracks().forEach(track => track.stop()); streamRef.current = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true } }); setPermission("ready"); setTypedMode(false); }
