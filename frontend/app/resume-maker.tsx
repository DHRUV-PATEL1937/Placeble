...
type Job = { id: string; status: "queued" | "processing" | "complete" | "failed"; progress: number; message: string; result?: { resume?: Resume; score?: Resume["atsBreakdown"] & { atsScore: number }; fileUrl?: string; filename?: string; found?: { sections: number; characters: number } }; error?: string };
...
export function ResumeMaker({ user, accessToken, onBack }: { user: { name: string; email: string }; accessToken: string; onBack: () => void }) {
  ...
  return <div className="view-content inner-view resume-maker">
    ...
    <div className="rm-workspace">
      ...
      <aside className={`rm-preview-panel ${mobileTab === "preview" ? "mobile-active" : ""}`}><header><div><span>Live preview</span><small>A4 · updates as you type</small></div><div className="rm-template-picker">{templates.map(template => <button key={template.id} className={resume.template === template.id ? "active" : ""} onClick={() => updateResume(current => ({ ...current, template: template.id }))}><strong>{template.label}</strong><small>{template.note}</small></button>)}</div></header><div className="rm-paper-stage"><ResumePreview resume={resume} user={user} /></div></aside>
    </div>
