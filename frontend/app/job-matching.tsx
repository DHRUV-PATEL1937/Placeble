...
export function JobMatching({ accessToken, onBack }: { accessToken: string; onBack: () => void }) {
  const [dashboard, setDashboard] = useState<Dashboard | null>(null);
  const [surface, setSurface] = useState<"feed" | "tracker">("feed");
  ...
  const loadDashboard = useCallback(async () => {
    const next = await api<Dashboard>("/matching/dashboard", accessToken);
    setDashboard(next);
  }, [accessToken]);

  useEffect(() => {
    const timer = window.setTimeout(() => void loadDashboard().catch(cause => setError(cause instanceof Error ? cause.message : "Could not load job matches.")).finally(() => setLoading(false)), 0);
  ...
  const updateStatus = async (application: Application, status: Status) => {
    ...
    setBusy(true); setError("");
    try { await api(`/matching/applications/${application._id}`, accessToken, { method: "PATCH", body: JSON.stringify({ status }) }); await loadDashboard(); setNotice(`${application.job.title} moved to ${statusMeta[status].label}.`); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "The application status could not be saved."); }
  ...
  const saveNotes = async () => {
    ...
    setBusy(true); setError("");
    try { await api(`/matching/applications/${selectedApplication._id}`, accessToken, { method: "PATCH", body: JSON.stringify({ notes: notesDraft }) }); await loadDashboard(); setSelectedApplication(null); setNotice("Your private application notes were saved."); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Notes could not be saved."); }
    finally { setBusy(false); }
  };
  ...
  return <div className="view-content inner-view job-matching-view"><header className="jm-module-top"><button onClick={onBack}><ArrowLeft size={17} /> All agents</button><div><span><Target size={16} /></span><strong>Job Matching</strong></div></header>
    ...
    </section> : <ApplicationTracker applications={dashboard?.applications ?? []} statusFilter={statusFilter} setStatusFilter={setStatusFilter} busy={busy} onStatus={updateStatus} onNotes={application => { setSelectedApplication(application); setNotesDraft(application.notes ?? ""); }} />}
...
function ApplicationTracker({ applications, statusFilter, setStatusFilter, busy, onStatus, onNotes }: { applications: Application[]; statusFilter: Status | "all"; setStatusFilter: (status: Status | "all") => void; busy: boolean; onStatus: (application: Application, status: Status) => void; onNotes: (application: Application) => void }) {
  ...
  return <section className="jm-tracker"><header><div><p className="eyebrow">Application tracker</p><h2>Keep every next step visible.</h2><p>Drag cards between columns on desktop or use the status selector anywhere.</p></div><label><Filter size={15} /><select value={statusFilter} onChange={event => setStatusFilter(event.target.value as Status | "all")}><option value="all">All statuses</option>{statuses.map(status => <option key={status} value={status}>{statusMeta[status].label}</option>)}</select></label></header><div className="jm-kanban">{statuses.filter(status => statusFilter === "all" || status === statusFilter).map(status => {
    ...
    return <section className={`jm-kanban-column ${status}`} key={status} onDragOver={event => event.preventDefault()} onDrop={event => { const id = event.dataTransfer.getData("application/id"); const application = applications.find(item => item._id === id); if (application) onStatus(application, status); }}><header><div><strong>{statusMeta[status].label}</strong><small>{statusMeta[status].copy}</small></div><span>{cards.length}</span></header><div>{cards.map(application => <article draggable={!busy} key={application._id} onDragStart={event => event.dataTransfer.setData("application/id", application._id)}><div className="jm-card-grip"><GripVertical size={15} /><span>{initials(application.job.companyName)}</span><em>{application.matchPercent}%</em></div><h3>{application.job.title}</h3><p>{application.job.companyName}</p><small><Clock3 size={12} />{application.daysSinceLastChange === 0 ? "Updated today" : `${application.daysSinceLastChange} days in ${statusMeta[application.status].label}`}</small><select aria-label={`Status for ${application.job.title}`} disabled={busy} value={application.status} onChange={event => onStatus(application, event.target.value as Status)}>{statuses.map(next => <option key={next} value={next}>{statusMeta[next].label}</option>)}</select><button onClick={() => onNotes(application)}><FileText size={13} />{application.notes ? "Edit notes" : "Add private note"}</button></article>)}{!cards.length && <div className="jm-column-empty"><Users size={18} /><span>No applications</span></div>}</div></section>;
  })}</div></section>;
}
