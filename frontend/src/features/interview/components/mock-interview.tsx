...
async function api<T>(path: string, accessToken: string, init: RequestInit = {}) {
  ...
  if (!response.ok) { const error = new Error(payload.message ?? "The request could not be completed.") as Error & { interviewId?: string }; error.interviewId = typeof payload.interviewId === "string" ? payload.interviewId : undefined; throw error; }
  return payload as T;
}
...
export function MockInterview({ accessToken, onBack }: { accessToken: string; onBack: () => void }) {
  ...
  const [blockedInterviewId, setBlockedInterviewId] = useState("");
  ...
  const requestMedia = async () => {
    ...
    try {
      streamRef.current?.getTracks().forEach(track => track.stop());
      const stream = await requestUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true } });
  ...
  const startSession = async () => {
    ...
    } catch (cause) { const blocking = cause as Error & { interviewId?: string }; setBlockedInterviewId(blocking.interviewId ?? ""); setError(cause instanceof Error ? cause.message : "Could not start this interview."); }
  ...
  if (screen === "setup") return <div className="view-content inner-view mock-interview-view"><InterviewTop label="Interview setup" onBack={() => setScreen("home")} />
    {error && <InterviewMessage type="error" text={error} onClose={() => { setError(""); setBlockedInterviewId(""); }} />}{blockedInterviewId && <section className="mi-active-session-action"><span><Pause size={18} /></span><div><strong>Continue your active interview</strong><p>Your progress is saved. Resume to continue with typed answers.</p></div><button className="button button-primary" disabled={busy} onClick={() => void resumeSession(blockedInterviewId)}><Play size={15} /> Resume interview</button></section>}
    <section className="mi-setup-intro"><div><p className="eyebrow">Before you walk into the room</p><h1>Settle in. This is practice, not performance.</h1><p>Your question appears as text first. Take a breath before you write—practice is focused on clear, thoughtful answers.</p></div><span><ShieldCheck size={22} /><strong>Private local session</strong><small>No recording permissions required.</small></span></section>
    <div className="mi-setup-grid"><section className="mi-device-card mi-typed-only-card"><header><div><p className="eyebrow">Answer format</p><h2>Answer your way</h2></div><span className="mi-permission ready"><CheckCircle2 size={15} /> Ready</span></header><div className="mi-camera-preview"><div><FileText size={30} /><strong>Write your answer at your own pace</strong><p>Use typed answers, or enable your microphone to answer naturally. Voice answers are transcribed securely by Sarvam before review.</p></div><span className="mi-preview-name">Private typed session</span></div><p className="mi-access-note"><Check size={14} /> Choose typed answers or enable a microphone for Sarvam transcription.</p><button className="mi-enable-voice" type="button" disabled={permission === "checking"} onClick={() => void requestMedia()}>{permission === "checking" ? <LoaderCircle size={15} /> : <Mic size={15} />}{permission === "ready" ? "Microphone ready" : "Enable voice answers"}</button></section>
      <section className="mi-config-card"><p className="eyebrow">Session design</p><h2>What should the interviewer focus on?</h2><label><span>Interview type</span><div className="mi-type-grid">{(Object.keys(typeMeta) as InterviewType[]).map(item => <button key={item} className={type === item ? "selected" : ""} onClick={() => setType(item)}><i>{type === item ? <Check size={14} /> : <Circle size={14} />}</i><strong>{typeMeta[item].label}</strong><small>{typeMeta[item].copy}</small></button>)}</div></label><label><span>Target role <em>optional</em></span><div className="mi-role-input"><BriefcaseBusiness size={17} /><input value={targetRole} onChange={event => setTargetRole(event.target.value)} placeholder="e.g. Product Analyst" maxLength={120} /></div></label><label><span>Number of questions</span><div className="mi-length-picker">{[3, 5, 6].map(value => <button key={value} className={totalTurns === value ? "selected" : ""} onClick={() => setTotalTurns(value)}>{value}<small>{value === 3 ? "Quick" : value === 5 ? "Standard" : "Deep"}</small></button>)}</div></label><button className="button button-primary mi-enter-room" disabled={busy} onClick={() => void startSession()}>{busy ? <LoaderCircle size={17} /> : <ArrowRight size={17} />} Enter interview room</button></section></div>
  </div>;
  ...
  return <div className="view-content inner-view mock-interview-view"><InterviewTop label="Mock Interview" onBack={onBack} />
    {error && <InterviewMessage type="error" text={error} onClose={() => setError("")} />}{notice && <InterviewMessage type="notice" text={notice} onClose={() => setNotice("")} />}
    <section className="mi-home-hero"><div><p className="eyebrow">Practice the room, not a script</p><h1>Build answers that sound clear, specific and genuinely yours.</h1><p>A calm, turn-by-turn typed interview with measured feedback and follow-up questions that respond to what you write.</p><div><button className="button button-primary" onClick={() => setScreen("setup")}><Mic2 size={17} /> Start mock interview</button>{completed[0] && <button className="button button-secondary" onClick={() => openInterview(completed[0], "debrief")}><Trophy size={17} /> View latest debrief</button>}</div><small><ShieldCheck size={14} /> Your question appears immediately. Voice playback never blocks the session.</small></div><aside><span><Mic2 size={24} /></span><p>Current interview signal</p><strong>{latestScore || 71}</strong><small>{completed.length ? "Latest completed session" : "Starter benchmark"}</small></aside></section>
    ...
    <section className="mi-how-it-works"><header><p className="eyebrow">A steady interview loop</p><h2>Know what is happening at every step.</h2></header><div>{[["01","Question first","Read immediately while voice prepares."],["02","Answer naturally","Write a clear, specific response."],["03","Measured review","Transcript, rubric score and adaptive follow-up."],["04","Full debrief","Patterns and practical next steps across the session."]].map(([number,title,copy]) => <article key={number}><span>{number}</span><strong>{title}</strong><p>{copy}</p></article>)}</div></section>
