...
export function MockInterview({ accessToken, onBack }: { accessToken: string; onBack: () => void }) {
  ...
  if (screen === "setup") return <div className="view-content inner-view mock-interview-view"><InterviewTop label="Interview setup" onBack={() => setScreen("home")} />
    ...
    <div className="mi-setup-grid"><section className="mi-device-card mi-typed-only-card"><header><div><p className="eyebrow">Answer format</p><h2>Typed-answer practice</h2></div><span className="mi-permission ready"><CheckCircle2 size={15} /> Ready</span></header><div className="mi-camera-preview"><div><FileText size={30} /><strong>Write your answer at your own pace</strong><p>Typed practice is temporarily the only interview mode. Your response is reviewed with the same structure, relevance, and specificity rubric.</p></div><span className="mi-preview-name">Private typed session</span></div><p className="mi-access-note"><Check size={14} /> No camera or microphone access is needed.</p></section>
