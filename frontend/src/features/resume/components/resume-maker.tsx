...
export function ResumeMaker({ user, accessToken, onBack }: { user: { name: string; email: string }; accessToken: string; onBack: () => void }) {
  ...
  const addEntry = (section: ResumeSection) => updateSection(section.type, { ...section.content, entries: [...(section.content.entries ?? []), section.type === "education" ? { degree: "New qualification", institution: "Institution", graduationYear: "2027", detail: "" } : section.type === "certifications" ? { title: "New certification", subtitle: "Issuing organization", date: "", detail: "" } : { title: "New entry", subtitle: "Organisation or context", date: "", bullets: [] }] });
